...
grep -rl 'org-or-user' . --exclude-dir=.git --exclude-dir=node_modules --exclude="*.sh" | xargs sed -i "s/ts-package-template/$name/g"
...
grep -rl 'ts-package-template' . --exclude-dir=.git --exclude-dir=node_modules --exclude="*.sh" | xargs sed -i "s/ts-package-template/$repo/g"
